...
from .rtdetr_modules import dct_2d, idct_2d, SpatialAttention, ChannelAttention, gen_class_guidance_map_max_prob_threshold, FEAdaptive
...
class FRM(nn.Module):
    def __init__(
        ...
        self.thresh_pred = nn.Sequential(
    ...
    @staticmethod
    def high_pass_filter_mask(size, thresh, device=None, sharpness=20.0):
        """
        High-pass filter mask using Straight-Through Estimator (STE).
        Forward: hard binary mask.
        Backward: gradient flows as if soft mask was used.
        """
        H, W = size
        B = thresh.shape[0]

        alpha_h = thresh[:, 0:1] * H  # float cutoff
        alpha_w = thresh[:, 1:2] * W

        h_coords = torch.arange(H, device=device).view(1, H, 1).expand(B, H, 1)
        w_coords = torch.arange(W, device=device).view(1, 1, W).expand(B, 1, W)

        # Soft mask for gradient (OR logic)
        soft_h = torch.sigmoid(sharpness * (h_coords - alpha_h.unsqueeze(-1)))
        soft_w = torch.sigmoid(sharpness * (w_coords - alpha_w.unsqueeze(-1)))
        soft_mask = 1 - (1 - soft_h) * (1 - soft_w)

        # Hard binary mask for forward pass
        hard_mask = ((h_coords >= alpha_h.unsqueeze(-1)) |
                    (w_coords >= alpha_w.unsqueeze(-1))).float()
    ...
    @staticmethod
    def low_pass_filter_mask(size, thresh, device=None, sharpness=20.0):
        """
        Low-pass filter mask using Straight-Through Estimator (STE).
        Forward: hard binary mask that keeps low frequencies (top-left)
        and blocks high frequencies (bottom-right).
        Backward: gradient flows as if soft mask was used.
        """
        H, W = size
        B = thresh.shape[0]
        ...
        # Soft mask for gradient (block bottom-right / high frequencies)
        soft_h = torch.sigmoid(sharpness * (h_coords - alpha_h.unsqueeze(-1)))
        soft_w = torch.sigmoid(sharpness * (w_coords - alpha_w.unsqueeze(-1)))
        soft_mask = 1 - (soft_h * soft_w)

        # Hard binary mask for forward pass (block only bottom-right corner)
        hard_mask = ((h_coords < alpha_h.unsqueeze(-1)) |
                     (w_coords < alpha_w.unsqueeze(-1))).float()
    ...
    def forward(self, feat):
        ...
        # freq: (B, C, H, W)
        B, C, H, W = freq.shape
        thresh = self.thresh_pred(F.adaptive_avg_pool2d(feat, (1, 1)).flatten(1))
        thresh_high, thresh_low = thresh[:, :2], thresh[:, 2:]
        mask_high = self.high_pass_filter_mask((H, W), thresh_high, device=freq.device)
        mask_low = self.low_pass_filter_mask((H, W), thresh_low, device=freq.device)
        # Expand mask to (B, 1, H, W) for broadcasting
        mask_high = mask_high[:, None, :, :]
        mask_low = mask_low[:, None, :, :]
        high_freq = freq * mask_high
        low_freq = freq * mask_low
        
        high_freq_feat = idct_2d(high_freq)
        low_freq_feat = idct_2d(low_freq)

        spatial_attn = self.spatial_att(high_freq_feat)
        channel_attn = self.channel_att(low_freq_feat)

        spatial_attn = feat * spatial_attn
        channel_attn = feat * channel_attn

        feat = self.outs(spatial_attn + channel_attn)
...
class RTDETRFPN(BaseModule):
    ...
    def __init__(
        ...
        for idx in range(len(in_channels) - 1, 0, -1):
            ...
            self.hfps.append(FRM(in_channels[idx - 1]))
    ...
    def forward(self, inputs: Tuple[Tensor]) -> Tuple[Tensor]:
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            inner_out = self.top_down_blocks[len(self.in_channels) - 1 - idx](
                torch.cat([upsample_feat, self.hfps[len(self.in_channels) - 1 - idx](feat_low)], 1))
...
@MODELS.register_module()
class FRDETRHybridEncoder(BaseModule):
...
class FRDETRTransformerDecoder(DinoTransformerDecoder):
    ...
    def __init__(self,
                 ...
                 init_cfg: Union[dict] = None,
                 num_classes: int = None) -> None:
        self.num_classes = num_classes
        super().__init__(num_layers, layer_cfg, post_norm_cfg, return_intermediate, init_cfg=init_cfg)
    ...
    def _init_layers(self) -> None:
        ...
        self.fe = ModuleList([
            FEAdaptive(self.embed_dims, self.num_classes), FEAdaptive(self.embed_dims, self.num_classes), FEAdaptive(self.embed_dims, self.num_classes)
        ]) # This is currently hardcoded for 3 stage.

    def forward(self, query: Tensor, value: Tensor, key_padding_mask: Tensor,
                self_attn_mask: Tensor, reference_points: Tensor,
                spatial_shapes: Tensor, level_start_index: Tensor,
                valid_ratios: Tensor, reg_branches: nn.ModuleList, cls_branches: nn.ModuleList, batch_data_samples = None,
                **kwargs) -> Tuple[Tensor]:
        ...
        src_size = [[[math.ceil(batch_img_metas[i]['img_shape'][0]/8), math.ceil(batch_img_metas[i]['img_shape'][1]/8)],
                    [math.ceil(batch_img_metas[i]['img_shape'][0]/16), math.ceil(batch_img_metas[i]['img_shape'][1]/16)],
                    [math.ceil(batch_img_metas[i]['img_shape'][0]/32), math.ceil(batch_img_metas[i]['img_shape'][1]/32)]]
                    for i in range(len(batch_img_metas))] # B, 3, 2
        
        output_cls = cls_branches[self.num_layers](query).sigmoid()
        guidance_map = gen_class_guidance_map_max_prob_threshold(output_cls.detach(), reference_points, src_size, spatial_shapes) # B, num_value, num_classes

        for lid, layer in enumerate(self.layers):
            value = self.fe[lid](value, spatial_shapes, guidance_map)
            ...
            if lid < len(self.layers) - 1:
                output_cls = cls_branches[lid](query).sigmoid() # B, num_queries, num_classes
                guidance_map = gen_class_guidance_map_max_prob_threshold(output_cls.detach(), reference_points, src_size, spatial_shapes) # B, num_value, num_classes
