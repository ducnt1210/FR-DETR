...
class XMLDataset(BaseDetDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for img_id in img_ids:
            # Try multiple possible image extensions (lowercase and uppercase)
            candidate_file_name = None
            for ext in ['jpg', 'jpeg', 'png', 'JPG', 'JPEG', 'PNG']:
                candidate_rel = osp.join(self.img_subdir, f'{img_id}.{ext}')
                candidate_full = osp.join(self.sub_data_root, candidate_rel)
                if osp.exists(candidate_full):
                    candidate_file_name = candidate_rel
                    break
            # Fallback to .jpg if none found (keeps previous default behavior)
            file_name = candidate_file_name \
                if candidate_file_name is not None \
                else osp.join(self.img_subdir, f'{img_id}.jpg')
            # file_name = osp.join(self.img_subdir, f'{img_id}.png') # change to png for RTTS
